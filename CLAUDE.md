...
- `schemas/**` (except `schemas/README.md`) — generated by `mise run schemas` (`hack/crd2jsonschema`) from `config/crd/bases/`; CI's drift check fails a PR that edits a CRD type without also committing the regenerated schema
...
- **CRD field naming**: a strictly two-valued on/off toggle (e.g. `collapse`, `errorDisplay`, `indexing`, `hostUsers`, `ingress.enabled`) is a plain `bool`/`*bool` field, not a string enum — kube-api-linter's usual "prefer enums over bools" guidance was evaluated for this repo (see `api/v1alpha1`'s git history) and deliberately not followed here, since unquoted `true`/`false` enum values collide with YAML 1.1's own boolean literals and force users to quote them (`enabled: "true"`), which is worse ergonomics than a native bool. Field names stay affirmative nouns for the thing being controlled, never a negated verb (`disableX`, `hideX`) — a bool field itself may read as a verb (`hideVersion: true`) since there's no double-negative risk once the value is `true`/`false` rather than another word. Reserve a real string `+kubebuilder:validation:Enum` for anything that isn't strictly two-valued today and could plausibly gain another state later (`statusStyle`'s `dot`/`basic` is kept as an enum for exactly this reason, even though only two values exist now), or values mirroring homepage's own YAML vocabulary (`light`/`dark`, `row`/`column`, ...); those keep PascalCase for operator-native values and lowercase for homepage-derived ones.
