...
| `InfoWidget` | `piw` | One or many header-strip widgets (`spec.type` for a single widget, or `spec.widgets` for a whole dashboard's header strip in one object): `datetime`, `greeting`, or `openmeteo`, among others. |
