...
func newDeploymentTestInstance() *pagev1alpha1.Instance {
	return &pagev1alpha1.Instance{
		ObjectMeta: metav1.ObjectMeta{Name: testInstanceObjName, Namespace: deploymentTestNamespace},
		Spec:       pagev1alpha1.InstanceSpec{ContainerPort: 8080},
	}
}

// schemeWithoutInstance registers everything deploymentForInstance and
// reconcileDeployment need except pagev1alpha1, so
// ctrl.SetControllerReference(instance, dep, scheme) fails to look up the
// Instance's GroupVersionKind — reproducing deploymentForInstance's only
// error return without needing a contrived Instance/Deployment state.
func schemeWithoutInstance(t *testing.T) *runtime.Scheme {
	t.Helper()
...
func TestReconcileDeploymentDefineError(t *testing.T) {
	...
	instance := newDeploymentTestInstance()
	cl := fake.NewClientBuilder().WithScheme(clientScheme).WithObjects(instance).WithStatusSubresource(instance).Build()

	r := &InstanceReconciler{Client: cl, Scheme: schemeWithoutInstance(t), DashboardImage: testDashboardImage}

	_, handled, err := r.reconcileDeployment(t.Context(), instance)
	if !handled {
		t.Error("reconcileDeployment() handled = false, want true when deploymentForInstance fails")
	}
	if err == nil {
	...
	if cond := meta.FindStatusCondition(updated.Status.Conditions, typeAvailableInstance); cond == nil || cond.Status != metav1.ConditionFalse {
		t.Errorf("Instance status condition = %+v, want Available=False", cond)
	}
}
...
func TestReconcileDeploymentGetError(t *testing.T) {
	...
	r := &InstanceReconciler{Client: cl, Scheme: clientScheme, DashboardImage: testDashboardImage}
...
func seedDriftedDeployment(t *testing.T, r *InstanceReconciler, instance *pagev1alpha1.Instance) *appsv1.Deployment {
	t.Helper()
...
func TestReconcileDeploymentUpdateErrorRefetchInstanceError(t *testing.T) {
	clientScheme := networkTestScheme(t)
	instance := newDeploymentTestInstance()
	...
	base := fake.NewClientBuilder().WithScheme(clientScheme).WithObjects(instance).WithStatusSubresource(instance).Build()
	r := &InstanceReconciler{Client: base, Scheme: clientScheme, DashboardImage: testDashboardImage}
