...
func TestLabelsForInstanceIncludesSelectorLabelsPlusVersion(t *testing.T) {
	labels := labelsForInstance("example.com/kubepage-operator@sha256:725beb947b49ab1c6f25a6aeabc2a7288e5a58e341477ee1eb2b54fa37178c7")

	for k, v := range selectorLabelsForInstance() {
		if labels[k] != v {
			t.Errorf("labelsForInstance()[%q] = %q, want %q (from selectorLabelsForInstance)", k, labels[k], v)
		}
	}
	version := labels["app.kubernetes.io/version"]
	if len(version) > maxLabelValueLen {
		t.Errorf("labelsForInstance() version label len = %d, exceeds Kubernetes limit of %d", len(version), maxLabelValueLen)
	}
}
