...
// These specs verify the ServiceEntry CRD schema CEL rule
// (api/v1alpha1/servicecard_types.go's XValidation marker on that struct)
// rejects a services entry that sets more than one of ping/siteMonitor/
...
var _ = Describe("ServiceCard monitor-source CRD schema validation", func() {
	...
	It("rejects ping + podSelector both set on a services entry", func() {
	...
	It("rejects siteMonitor + podSelector both set on a services entry", func() {
...
// services entry carries the given combination of ping/siteMonitor/
// podSelector (any may be nil).
	...
	return &pagev1alpha1.ServiceCard{
		...
		Spec: pagev1alpha1.ServiceCardSpec{
			...
			Services: []pagev1alpha1.ServiceEntry{
				{Name: name, Ping: ping, SiteMonitor: siteMonitor, PodSelector: sel},
			},
		},
	}
}
