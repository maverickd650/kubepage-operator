...
func newNetworkPolicyTestInstance(ns string) *pagev1alpha1.Instance {
...
func TestNetworkPolicyForInstanceUnrestrictedIngressNoEgressByDefault(t *testing.T) {
	...
	instance := newNetworkPolicyTestInstance("np-ns")
...
func TestReconcileNetworkPolicyLifecycle(t *testing.T) {
	...
	// Disabling removes the NetworkPolicy.
	instance.Spec.NetworkPolicy.Enabled = pagev1alpha1.Disabled
	if err := r.reconcileNetworkPolicy(t.Context(), instance); err != nil {
		t.Fatalf("reconcileNetworkPolicy() delete error = %v", err)
	}
	err := cl.Get(t.Context(), types.NamespacedName{Name: instance.Name, Namespace: instance.Namespace}, &np)
	if err == nil {
		t.Fatal("NetworkPolicy still exists after disabling spec.networkPolicy.enabled")
	}
}
