...
// This spec verifies that InfoWidgetReconciler sets status.entries to the
// number of entries InfoWidgetSpec.Entries() resolves (len(spec.widgets)),
// so `kubectl get piw`'s Entries printcolumn reads correctly.
var _ = Describe("InfoWidget status.entries", func() {
	ctx := context.Background()

	It("sets entries to len(widgets)", func() {
