...
// These specs verify that BookmarkReconciler sets status.entries to the
// number of entries BookmarkSpec.Entries() resolves — 1 for the
// single-bookmark form, len(spec.bookmarks) for the multi-bookmark form — so
// `kubectl get pbmk`'s Entries printcolumn reads correctly for both forms.
var _ = Describe("Bookmark status.entries", func() {
	ctx := context.Background()

	It("sets entries to 1 for the single-bookmark form", func() {
		name := "bm-entries-single"
	...
	It("sets entries to len(bookmarks) for the multi-bookmark form", func() {
